...
\textit{Предикат}~--- это суждение о субъекте или субъектах, которое может быть задано логической формулой или иным образом. Предикат задаёт логическую функцию с аргументом (аргументами) и обращется в истину или ложь при задании аргументов. \textit{Примеры предикатов}: $\mathrm{blue}(x)=\text{$x$ -- синий предмет}$, $\mathrm{odd}(x)=\text{$x$ -- нечётное число}$, $\mathrm{nightmare}(x) = \mathrm{blue}(x)\lor\mathrm{odd}(x)$.
